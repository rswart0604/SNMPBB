...
gnd = [ones(60,1); 2*ones(140,1); 3*ones(100,1)];  % graph.data, knn = 15
% gnd = [ones(3000,1); 2*ones(3000,1); 3*ones(3000,1)];  % new_graph2.data, knn = 100
% gnd = [ones(500,1); 2*ones(500,1); 3*ones(500,1)];  % new_graph.data, knn = 40
% gnd = [ones(1000,1); 2*ones(1000,1); 3*ones(1000,1)];  % new_graph3.data, knn = 80
% gnd = [ones(75,1); 2*ones(75,1); 3*ones(75,1); 4*ones(75,1)];  % clustered_data.data
...
plot_time_limit = 7; % Show only first 30 seconds (adjust as needed)
...
plot(t_plot, mean(acc_anls_all(:,time_idx),1),'Color','#8516D1','LineWidth',2)
plot(t_plot, mean(acc_pgd_all(:,time_idx),1),'Color','#3BAA32','LineWidth',2)
plot(t_plot, mean(acc_newton_all(:,time_idx),1),'Color','#2FBEEF','LineWidth',2)
ax = gca;
ax.FontSize = 14;
